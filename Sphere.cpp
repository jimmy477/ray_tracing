...
{
    glm::vec3 vdif = p0 - center;   //Vector s (see Slide 28), vector to the center of sphere
    float b = glm::dot(dir, vdif);  //Angle between dir of ray and the vector from point to center of sphere
    float len = glm::length(vdif);
    float c = len*len - radius*radius;
    float delta = b*b - c;          //Discriminant
   
    //fabs(x) is the absolute value of x
